...
let DashboardComponent = class DashboardComponent {
    ...
    ngOnInit() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            yield this.worker.getWorker().then(data => {
                data.subscribe(data => {
                ...
                data["orders"].map(order => {
                    this.WorkersList.map(worker => {
                        ...
                        if (worker.id === order["workerId"]) {
                            ...
                            this.Workers.push(Object.assign({ fullName: this.name }, worker, order));
                        }
                    });
                    this.sortFunction();
                });
...
let FilterSearchPipe = class FilterSearchPipe {
    ...
    transform(Workers, search) {
        if (!Workers || !search) {
            return Workers;
        }
        return Workers.filter(worker => worker.fullName.toLowerCase().indexOf(search.toLowerCase()) !== -1);
    }
};
...
WorkOrderService.ctorParameters = () => [
